...
namespace ConstructionLine.CodingChallenge.Tests
{
    ...
    {
        public readonly IShirtSearch _shirtSearch = new ShirtSearch();
        public readonly IResultsProcessor _resultsProcessor = new ResultsProcessor();
        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            foreach (var size in Size.All)
            {
                ...
                // If we have no search options we can expect nothing
                expectedSizeCount = !searchOptions.Colors.Any() && !searchOptions.Sizes.Any() ? 0 : expectedSizeCount;
        ...
        protected static void AssertColorCounts(List<Shirt> shirts, SearchOptions searchOptions, List<ColorCount> colorCounts)
        {
            ...
            foreach (var color in Color.All)
            {
                ...
                // If we have no search options we can expect nothing
                expectedColorCount = !searchOptions.Colors.Any() && !searchOptions.Sizes.Any() ? 0 : expectedColorCount;
